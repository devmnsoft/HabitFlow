...
export async function reportFrontendError(error, context = {}) { const metadata = errorMetadata(error, { ...(context.metadata || {}), action: context.action || "unknown" }); const payload = { type: "frontend_error", severity: context.severity || "error", source: context.source || "frontend", action: context.action || "unknown", message: String(context.message || error?.message || "Erro frontend capturado.").slice(0, 500), metadata }; await registerUsageEvent(auth.currentUser, "frontend_error", payload); if (shouldIgnoreForRemoteReport(error, context) || shouldDedupe(error, context)) { enqueuePendingLog(payload); return; } await logSystemEvent(payload).catch(() => {}); }
export async function logSystemEvent({ type, severity = "info", source = "frontend", action, message, metadata = {} }) { const payload = { type, severity, source, action: action || type, message: String(message || "Evento registrado.").slice(0, 500), metadata: sanitizeMetadata(metadata) }; const result = await callFunction("logSystemEvent", payload, { silent: true }); if (!result.ok) { enqueuePendingLog(payload); if (APP_ENV === "development") console.warn("[HabitFlow] logSystemEvent falhou; evento salvo localmente", result.code); return null; } return result.data; }
export async function trackUserAction(action, metadata = {}, options = {}) { const user = auth.currentUser; if (!user) return; const safeMetadata = sanitizeMetadata(metadata); try { await firebaseApi.addDoc(firebaseApi.collection(db, "users", user.uid, "usageEvents"), { userId: user.uid, userEmail: user.email || "", createdAt: firebaseApi.serverTimestamp(), type: action, action, source: "frontend", metadata: safeMetadata, appVersion: APP_VERSION, environment: APP_ENV }); } catch (error) { if (APP_ENV === "development") console.warn("[HabitFlow] usage event falhou", error?.code || error?.message || error); } const globalActions = new Set(["login", "signup", "logout", "terms_accepted", "habit_created", "habit_updated", "habit_archived", "habit_restored", "habit_completed", "habit_uncompleted", "premium_interest", "premium_checkout_clicked", "checkout_success_return", "checkout_pending_return", "checkout_failure_return", "admin_panel_opened", "app_loaded", "dashboard_loaded", "habits_loaded"]); if (options.global !== false && globalActions.has(action)) await logSystemEvent({ type: action.startsWith("login") ? "user_login" : action === "signup" ? "user_signup" : action, severity: options.severity || (safeMetadata?.durationMs > 3000 ? "warning" : "info"), source: options.source || "frontend", action, message: options.message || `Ação registrada: ${action}`, metadata: safeMetadata }); }
