...
const output = path.resolve('artifacts/header-context/v6117/after');
const rows = [];
...
export async function assertDrawerWorks(page, width) { if(width>=1024)return; await page.locator('.header-v4__menu').click(); await expect(page.locator('#headerDrawer')).toHaveClass(/show/); await page.keyboard.press('Escape'); }
...
test.afterAll(async()=>fs.writeFile(path.resolve('artifacts/header-context/v6117/report.md'),`# Evidência visual do header v6.11.7\n\n| Rota | Viewport | Contexto | Visíveis | Ocultos | Overflow | Overlap | Screenshot | Resultado |\n|---|---|---|---|---|---|---|---|---|\n${rows.join('\n')}\n`));
