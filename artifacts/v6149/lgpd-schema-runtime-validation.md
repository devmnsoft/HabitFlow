...
O container não possui `pwsh` nem `psql`, e nenhuma connection string local foi fornecida. Não foi fabricada evidência de banco. O validador v6.14.9 foi endurecido para conferir as quatro colunas obrigatórias de `user_privacy_consents`, as cinco colunas de `privacy_request_events` (tipo e nulabilidade), a FK para `lgpd_requests`, o índice de consulta e o trigger `trg_audit_privacy_request`, além das tabelas já verificadas.
