...
with required_tables(table_name) as (values
 ...
 ('billing_events'),('notifications'),('user_reports')
 ...
 ('ix_habitflow_system_audit_logs_created_at'),('ix_habitflow_admin_audit_logs_created_at')
 ...
 ('ck_habitflow_billing_events_plan')
), public_conflicts as (
 select table_schema, table_name from information_schema.tables where table_schema = 'public' and table_name in
 ('users','habits','habit_completions','support_tickets','support_messages','system_audit_logs','admin_audit_logs','system_settings','lgpd_requests','billing_events','notifications','user_reports')
), missing_tables as (
...
begin
 ...
 if exists(select 1 from information_schema.tables where table_schema='public' and table_name in ('users','habits','habit_completions','support_tickets','support_messages','system_audit_logs','admin_audit_logs','system_settings','lgpd_requests','billing_events','notifications','user_reports')) then errors := array_append(errors,'há tabelas HabitFlow indevidas no schema public'); end if;
 if exists(with r(table_name) as (values ('users'),('habits'),('habit_completions'),('support_tickets'),('support_messages'),('system_audit_logs'),('admin_audit_logs'),('system_settings'),('lgpd_requests'),('billing_events'),('notifications'),('user_reports')) select 1 from r left join information_schema.tables t on t.table_schema='habitflow' and t.table_name=r.table_name where t.table_name is null) then errors := array_append(errors,'há tabelas obrigatórias ausentes em habitflow'); end if;
