...
public sealed record SupportTicketDetail(Guid Id, Guid ClientId, Guid UserId, string Protocol, string Category, string Status, string Subject, string Description, string SafeContext, DateTime CreatedAt, DateTime UpdatedAt, DateTime? ClosedAt);
public sealed record SupportTicketMessage(Guid Id, Guid ClientId, Guid TicketId, Guid UserId, bool IsStaff, string Message, DateTime CreatedAt);
