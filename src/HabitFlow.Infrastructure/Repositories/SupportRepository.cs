...
{
    public Task CreateTicketAsync(SupportTicket ticket, CancellationToken ct = default) => db.ExecuteAsync("insert into habitflow.support_tickets(id,user_id,protocol,type,status,priority,title,description,source,created_at,updated_at,resolved_at) values(@Id,@UserId,@Protocol,@Type,@Status,@Priority,@Title,@Description,@Source,@CreatedAt,@UpdatedAt,@ResolvedAt)", new { ticket.Id, ticket.UserId, ticket.Protocol, ticket.Type, Status = DbEnum.Text(ticket.Status), ticket.Priority, ticket.Title, ticket.Description, ticket.Source, ticket.CreatedAt, ticket.UpdatedAt, ticket.ResolvedAt }, ct);
