...
public sealed class SupportController(SupportCenterService service,ILogger<SupportController> logger):Controller
{
    ...
    [HttpPost("tickets/new"),ValidateAntiForgeryToken] public async Task<IActionResult> Create(string category,string priority,string subject,string description,string? currentRoute,string? viewport,CancellationToken ct){if(string.IsNullOrWhiteSpace(subject)||string.IsNullOrWhiteSpace(description)){ModelState.AddModelError("","Título e descrição são obrigatórios.");return await New(ct);}var id=await service.CreateAsync(this.CurrentClientId(),this.CurrentUserId(),category,priority,subject,description,currentRoute??Request.Path,Request.Headers.UserAgent.ToString(),viewport??"não informado",this.CurrentUserSnapshot().Plan.ToString(),HttpContext.TraceIdentifier,ct);return RedirectToAction(nameof(Detail),new{id});}
    [HttpGet("tickets/{id:guid}")] public async Task<IActionResult> Detail(Guid id,CancellationToken ct){var ticket=await service.GetAsync(this.CurrentClientId(),this.CurrentUserId(),id,Admin,ct);if(ticket is null)return NotFound();return View(new TicketDetailViewModel(ticket,await service.MessagesAsync(this.CurrentClientId(),id,Admin,ct),await service.ContactAsync(ct)));}
    [HttpPost("tickets/{id:guid}/reply"),ValidateAntiForgeryToken] public async Task<IActionResult> Reply(Guid id,string message,CancellationToken ct){if(!await service.ReplyAsync(this.CurrentClientId(),this.CurrentUserId(),id,Admin,message,false,ct))TempData["Error"]="Não foi possível enviar. Confira a mensagem e o status do chamado.";return RedirectToAction(nameof(Detail),new{id});}
