...
{
    private static readonly EventId DashboardViewed = new(617801, "analytics.dashboard.viewed");
    private static readonly EventId InvalidFilter = new(617802, "analytics.invalid_filter");
        ...
        [FromQuery] bool? reminder, [FromQuery] bool? completed, CancellationToken ct)
    {
        ...
        try
        {
            ...
            logger.LogInformation(DashboardViewed,"analytics.dashboard.viewed Code={Code} CorrelationId={CorrelationId} ReportType={ReportType} Period={Start}:{End} Result={Result} DurationMs={DurationMs}","ANALYTICS_DASHBOARD_VIEWED",HttpContext.TraceIdentifier,"my-evolution",period.Start,period.End,"success",timer.ElapsedMilliseconds);
            return View(model);
        }
