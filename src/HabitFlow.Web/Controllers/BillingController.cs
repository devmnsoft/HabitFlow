...
public sealed class BillingController(SubscriptionService subscriptions, PaymentCheckoutService checkout, IPaymentTransactionRepository transactions) : Controller
{
    ...
    [HttpGet("billing/return/success")] public IActionResult Success() => View("Return", "Pagamento recebido pelo checkout. O Premium será ativado somente após confirmação segura do backend/webhook.");
    [HttpGet("billing/return/pending")] public IActionResult Pending() => View("Return", "Pagamento pendente. Avisaremos quando o provedor confirmar.");
