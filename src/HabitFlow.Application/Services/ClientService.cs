...
public sealed class ClientService(IClientRepository repo, AuditService audit, DocumentValidator documents, ILogger<ClientService> logger)
{
    public async Task<Result<Client>> CreateAsync(CreateClientRequest request, User adminUser, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var normalized = documents.Normalize(request.Document);
            var client = new Client(Guid.NewGuid(), Clean(request.Name)!, Clean(request.LegalName), normalized, Clean(request.Email), Clean(request.Phone), Clean(request.ContactName), request.Plan, request.Status, Clean(request.Notes), request.Status == ClientStatus.Active, now, now, request.PersonType, request.DocumentType, Clean(request.Document), normalized, Clean(request.TradeName), null, null, Clean(request.BillingEmail), Clean(request.BillingPhone), Clean(request.BillingResponsibleName), Clean(request.AddressZipcode), Clean(request.AddressStreet), Clean(request.AddressNumber), Clean(request.AddressComplement), Clean(request.AddressDistrict), Clean(request.AddressCity), Clean(request.AddressState));
    ...
    public async Task<Result<Client>> UpdateAsync(Guid id, UpdateClientRequest request, User adminUser, CancellationToken ct)
    {
        ...
        {
            var normalized = documents.Normalize(request.Document);
            var updated = existing with { Name = Clean(request.Name)!, LegalName = Clean(request.LegalName), Document = normalized, Email = Clean(request.Email), Phone = Clean(request.Phone), ContactName = Clean(request.ContactName), Plan = request.Plan, Status = request.Status, Notes = Clean(request.Notes), IsActive = request.Status == ClientStatus.Active, UpdatedAt = DateTime.UtcNow, PersonType = request.PersonType, DocumentType = request.DocumentType, DocumentRaw = Clean(request.Document), DocumentNormalized = normalized, TradeName = Clean(request.TradeName), BillingEmail = Clean(request.BillingEmail), BillingPhone = Clean(request.BillingPhone), BillingResponsibleName = Clean(request.BillingResponsibleName), AddressZipcode = Clean(request.AddressZipcode), AddressStreet = Clean(request.AddressStreet), AddressNumber = Clean(request.AddressNumber), AddressComplement = Clean(request.AddressComplement), AddressDistrict = Clean(request.AddressDistrict), AddressCity = Clean(request.AddressCity), AddressState = Clean(request.AddressState) };
    ...
    private async Task<string?> ValidateAsync(ClientRequestBase r, Guid? id, CancellationToken ct) { if (string.IsNullOrWhiteSpace(r.Name)) return "Informe o nome do cliente."; if (r.Name.Length > 180) return "O nome deve ter no máximo 180 caracteres."; if (!string.IsNullOrWhiteSpace(r.Email) && !Regex.IsMatch(r.Email, @"^[^@\s]+@[^@\s]+\.[^@\s]+$")) return "Informe um e-mail válido."; if (!string.IsNullOrWhiteSpace(r.Phone) && r.Phone.Length > 40) return "O telefone deve ter no máximo 40 caracteres."; if (r.PersonType == ClientPersonType.NaturalPerson && r.DocumentType != ClientDocumentType.CPF) return "Pessoa física deve usar CPF."; if (r.PersonType == ClientPersonType.LegalPerson && r.DocumentType != ClientDocumentType.CNPJ) return "Pessoa jurídica deve usar CNPJ."; var normalized = documents.Normalize(r.Document); if (r.DocumentType == ClientDocumentType.CPF && !documents.ValidateCpf(normalized)) return "Informe um CPF válido."; if (r.DocumentType == ClientDocumentType.CNPJ && !documents.ValidateCnpj(normalized)) return "Informe um CNPJ válido."; if (await repo.DocumentExistsAsync(normalized, id, ct)) return "Já existe um cliente cadastrado com este CPF/CNPJ."; return null; }
