...
public sealed class PlanService(IPlanRepository plans, ISubscriptionRepository subscriptions, ILogger<PlanService> logger)
{
    ...
    public async Task<Result<Plan>> GetUserEffectivePlanAsync(Guid userId, CancellationToken ct = default) { var sub = await subscriptions.GetActiveOrLatestByUserIdAsync(userId, ct); var code = sub is { Status: SubscriptionStatus.Active or SubscriptionStatus.Trial or SubscriptionStatus.Trialing or SubscriptionStatus.PastDue } ? sub.PlanCode : "free"; return await GetPlanByCodeAsync(code, ct); }
}
...
public sealed class PaymentCheckoutService(IPlanRepository plans, IPlanCatalogRepository catalog, SubscriptionService subscriptions, IPaymentProviderService provider, IPaymentAuditRepository audit, IConfiguration configuration, ILogger<PaymentCheckoutService> logger)
{
    public async Task<Result<CheckoutPreference>> StartCheckoutAsync(Guid userId, string email, string name, string planCode, BillingCycle cycle, CancellationToken ct = default)
    {
        try
        {
            var enabled = configuration.GetValue<bool>($"{PaymentsOptions.SectionName}:Enabled");
            var configuredProvider = configuration[$"{PaymentsOptions.SectionName}:Provider"] ?? "Manual";
            if (!enabled || !string.Equals(configuredProvider, "MercadoPago", StringComparison.OrdinalIgnoreCase))
            {
                logger.LogWarning(new EventId(6210, "billing.checkout.unavailable"), "billing.checkout.unavailable Provider={Provider} Plan={PlanCode}", configuredProvider, planCode);
                await audit.CreateAsync(new(Guid.NewGuid(), userId, null, "billing.checkout.unavailable", "Checkout indisponível; encaminhado ao atendimento comercial.", "Warning", null, DateTime.UtcNow), ct);
                return Result<CheckoutPreference>.Failure("payment.checkout_unavailable", "Checkout indisponível neste ambiente. Fale com a MNSOFT pelo e-mail comercial@mnsoft.com.br.");
            }
