...
public sealed record GoalListViewModel(IReadOnlyList<GoalCardViewModel> Goals,string? Search,string Status,string Sort,int ActiveCount,int PausedCount,int CompletedCount);
...
{
 public async Task<GoalListViewModel> ListAsync(Guid c,Guid u,string? search,string? status,string? sort=null,CancellationToken ct=default){var all=await goals.ListAsync(c,u,ct);var normalizedStatus=string.IsNullOrWhiteSpace(status)?"All":status;var normalizedSort=string.IsNullOrWhiteSpace(sort)?"recent":sort;var filtered=all.Where(g=>(normalizedStatus=="All"||g.Status.Equals(normalizedStatus,StringComparison.OrdinalIgnoreCase))&&(string.IsNullOrWhiteSpace(search)||g.Title.Contains(search,StringComparison.OrdinalIgnoreCase)||(g.Description?.Contains(search,StringComparison.OrdinalIgnoreCase)??false))).ToList();var cards=new List<GoalCardViewModel>();foreach(var goal in filtered){var linked=await goals.ListLinkedHabitsAsync(goal.Id,c,u,ct);cards.Add(new(goal,progress.Build(goal),linked.Count));}cards=normalizedSort switch{"progress-asc"=>cards.OrderBy(x=>x.Progress.Percentage).ToList(),"progress-desc"=>cards.OrderByDescending(x=>x.Progress.Percentage).ToList(),"deadline"=>cards.OrderBy(x=>x.Goal.EndDate??DateOnly.MaxValue).ToList(),"linked"=>cards.OrderByDescending(x=>x.LinkedHabitCount>0).ThenByDescending(x=>x.Goal.CreatedAt).ToList(),"unlinked"=>cards.OrderBy(x=>x.LinkedHabitCount>0).ThenByDescending(x=>x.Goal.CreatedAt).ToList(),_=>cards.OrderByDescending(x=>x.Goal.CreatedAt).ToList()};return new(cards,search,normalizedStatus,normalizedSort,all.Count(g=>g.Status=="Active"),all.Count(g=>g.Status=="Paused"),all.Count(g=>g.Status=="Completed"));}
