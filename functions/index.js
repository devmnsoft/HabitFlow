...
const VALID_SOURCES = ["frontend", "backend", "function", "payment", "auth", "firestore", "pwa"];
...
async function writeSystemAuditLog(event) {
  ...
  const payload = {
    ...
    telegramSent: false, errorFingerprint: event.errorFingerprint || errorFingerprint(metadata, event.message)
  };
...
function publicLog(r) { return { id: r.id, type: r.type, severity: r.severity, source: r.source, userId: r.userId, userEmail: r.userEmail, userName: r.userName, action: r.action, message: r.message, createdAt: r.createdAt, metadata: sanitizeMetadata(r.metadata || {}), appVersion: r.appVersion, environment: r.environment, readByAdmin: r.readByAdmin, telegramSent: r.telegramSent, errorFingerprint: r.errorFingerprint }; }
