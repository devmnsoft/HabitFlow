...
$ErrorActionPreference='Stop'; $root=(Resolve-Path (Join-Path $PSScriptRoot '../..')).Path; $report=Join-Path $root 'artifacts/v6132/smoke-routes-report.md'; New-Item -ItemType Directory -Force (Split-Path $report)|Out-Null
...
try {
  ...
  @('# Smoke de rotas autenticadas v6.13.2','',"Executado em: $(Get-Date -Format o)",'','| Rota | Status | Evidência |','|---|---|---|')+$rows|Set-Content $report -Encoding utf8
