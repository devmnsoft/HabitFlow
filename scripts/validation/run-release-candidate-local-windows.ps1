...
<#
.SYNOPSIS
Runs the HabitFlow v6.14.9 release gate on a Windows development workstation.
.EXAMPLE
...
Set-Location $root
$artifactDir = Join-Path $root 'artifacts/v6149'
$publishDir = Join-Path $root 'artifacts/v6149-publish'
...
function New-MobileChecklist {
  if($SkipMobileChecklist){Write-Report 'mobile-minimum-validation.md' @('# Checklist mobile v6.14.9','','Não executado: `-SkipMobileChecklist` foi informado.','', '**P0 pendente: validação visual não realizada.**');return}
  ...
  $lines=@('# Checklist mobile v6.14.9','',"Gerado em: $(Get-Date -Format o)",'','Preencher manualmente em navegador real. Nenhuma captura foi fabricada.','','## Viewports') + ($viewports|ForEach-Object{"- [ ] $_"}) + @('','## Telas') + ($screens|ForEach-Object{"- [ ] $_ — sem overflow, CTA coberto, header quebrado, modal fora da tela ou contraste crítico"}) + @('','**P0 pendente até que todos os itens sejam conferidos.**')
...
try {
  ...
  $precheck=@('# Pré-check Windows v6.14.9','',"Executado em: $(Get-Date -Format o)",'- Sistema operacional: Windows real','- A senha da conexão não é registrada.','', '| Ferramenta | Status | Versão / instalação |','|---|---|---|')+$toolRows
  ...
  Write-Report 'build-publish-validation.md' @('# Build e publish v6.14.9','',"Executado em: $(Get-Date -Format o)",'','- clean: aprovado','- restore: aprovado','- build: aprovado',"- publish: aprovado em diretório ignorado `$publishDir`")

  $currentPhase='migrations e EXPLAIN'
  Invoke-Step 'migrations' {& (Join-Path $root 'scripts/validation/validate-postgres-migrations.ps1') -ConnectionString $ConnectionString -TemporaryDatabase habitflow_v6149_fresh -ReportPath 'artifacts/v6149/postgres-migrations-validation.md'}
...
limit 1
"@
  Write-Report 'habit-template-explain-validation.md' @('# EXPLAIN da consulta de templates favoritos v6.14.9','',"Executado em: $(Get-Date -Format o)",'','## Tipos observados','```text',$columnTypes,'```','','## Plano','```text',$templateExplain,'```','','A consulta foi aceita pelo PostgreSQL com `ON_ERROR_STOP=1`; nenhum identificador ou valor de usuário foi persistido.')
  ...
  Write-Report 'startup-runtime-log-validation.md' @('# Startup v6.14.9','',"Executado em: $(Get-Date -Format o)","- URL: $BaseUrl",'- Aplicação publicada respondeu em até 120 segundos.','- Log verificado contra assinaturas técnicas proibidas.')
  ...
  Write-Report 'public-smoke-validation.md' (@('# Smoke público v6.14.9','',"Executado em: $(Get-Date -Format o)",'','| Rota | HTTP | Status |','|---|---:|---|')+$publicRows)
  ...
  Write-Report 'dev-user-seed-validation.md' @('# Usuário e seed v6.14.9','',"Executado em: $(Get-Date -Format o)","- Usuário: $DevEmail",'- Senha aleatória permaneceu somente em memória como SecureString.','- Seed idempotente aplicado e IDs resolvidos.')
  & scripts/validation/smoke-authenticated-routes.ps1 -BaseUrl $BaseUrl -Email $DevEmail -Password $securePassword -HabitId $ids[0] -TemplateId $ids[1] -GoalId $ids[2] -ReportPath 'artifacts/v6149/authenticated-smoke-validation.md';$results['smoke autenticado']='Aprovado';Assert-RuntimeLog
  Write-Report 'habit-library-manual-validation.md' @('# Biblioteca de hábitos v6.14.9','',"Gerado em: $(Get-Date -Format o)",'- Rotas de biblioteca, favoritos, detalhe e customização: aprovadas pelo smoke autenticado.','- Favoritar/desfavoritar, filtros, criação por template e reload: pendentes de conferência manual em navegador real.','- Nenhuma aprovação manual ou screenshot foi fabricada.','','**P0 pendente até a conferência manual.**')
  ...
  Write-Report 'mvp-essential-journey-validation.md' @('# Jornada MVP v6.14.9','',"Executado em: $(Get-Date -Format o)",'','Validação automatizada: login, rotas, reload e persistência do seed.','',"Contagens (hábitos ativos|conclusões hoje|objetivos|lembretes|notificações): `$counts`",'','Interações completas de criação/customização devem ser conferidas manualmente; não são declaradas aprovadas sem navegador real.')
  Write-Report 'mvp-plan-rules-validation.md' @('# Regras do MVP v6.14.9','',"Executado em: $(Get-Date -Format o)",'','- Uso do plano e páginas comerciais: cobertos pelo smoke autenticado/público.','- Limite Free, sexto hábito, template e duplicação: **pendentes de execução transacional guiada**.','- Ritmo e Evolução: nenhuma alteração comercial foi feita pelo runner.','','**P0 pendente: regras mutacionais não foram comprovadas automaticamente.**')
  New-MobileChecklist
  Write-Report 'first-runtime-failure-fix.md' @('# Bugs observados v6.14.9','',"Executado em: $(Get-Date -Format o)",'','Nenhum bug de runtime pode ser registrado sem uma falha observada. Consulte o relatório final e o log bruto.')
} catch {
  $failure=$_.Exception.Message
  $results['falha']='Não aprovado'
  if(-not(Test-Path (Join-Path $artifactDir 'windows-precheck.md'))){Write-Report 'windows-precheck.md' @('# Ambiente v6.14.9','',"Executado em: $(Get-Date -Format o)","**P0:** $failure",'','Nenhuma ferramenta foi instalada automaticamente. Nenhuma senha foi registrada.')}
  Write-Report 'first-runtime-failure-fix.md' @('# Primeira falha real v6.14.9','',"Executado em: $(Get-Date -Format o)","- Fase: $currentPhase",'- Status: falha preservada; nenhuma exceção foi mascarada.','','## Erro bruto','```text',$failure,'```','','## Causa e correção','A causa deve ser diagnosticada no Windows real a partir deste erro. Nenhuma correção de aplicação é declarada sem reprodução e reexecução.')
  throw
} finally {
