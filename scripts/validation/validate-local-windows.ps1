...
$dir=Join-Path $root 'artifacts/v6131'; New-Item -ItemType Directory -Force $dir|Out-Null
...
try {
  ...
  Run 'dotnet publish Release' {dotnet publish src/HabitFlow.Web/HabitFlow.Web.csproj --configuration Release --no-build --output artifacts/v6131-publish}
  ...
  $env:ASPNETCORE_URLS=$BaseUrl; $process=Start-Process dotnet -ArgumentList 'artifacts/v6131-publish/HabitFlow.Web.dll' -PassThru -RedirectStandardOutput (Join-Path $dir 'app.stdout.log') -RedirectStandardError (Join-Path $dir 'app.stderr.log')
  ...
  @('# Validação local Windows v6.13.1','',"Base URL: $BaseUrl",'','| Etapa | Status |','|---|---|')+$steps|Set-Content $summary -Encoding utf8
} catch { @('# Validação local Windows v6.13.1','',"**Falhou:** $($_.Exception.Message)",'','A connection string nunca é registrada por este script.')|Set-Content $summary -Encoding utf8; throw }
