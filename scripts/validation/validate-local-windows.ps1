...
try {
  ...
  try { for($i=0;$i-lt 30;$i++){try{Invoke-WebRequest "$BaseUrl/" -UseBasicParsing|Out-Null;break}catch{Start-Sleep 1}}; foreach($route in '/','/plans','/login','/service-worker.js','/favicon.ico'){ $response=Invoke-WebRequest "$BaseUrl$route" -UseBasicParsing -MaximumRedirection 5; if($response.StatusCode -ge 400){throw "GET $route returned $($response.StatusCode)"}; $steps.Add("| GET $route | Aprovado ($($response.StatusCode)) |") }; try{$health=Invoke-WebRequest "$BaseUrl/health/ready" -UseBasicParsing;if($health.StatusCode -lt 400){$steps.Add('| GET /health/ready | Aprovado |')}}catch{$steps.Add('| GET /health/ready | Não executado (rota ausente) |')} } finally {Stop-Process $process -Force -ErrorAction SilentlyContinue}
